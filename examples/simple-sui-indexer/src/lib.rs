...
use crate::bot::reactor::try_init_reactor;
...
pub async fn run_indexer() -> Result<()> {
    ...
    let database_url = database_url_str
        .parse::<Url>()
    ...
    let bot_reactor = try_init_reactor(&database_url_str)
        .await?
        .map(|reactor| {
            info!("Bot reactor enabled");
            reactor
        });
    if bot_reactor.is_none() {
        info!("Bot reactor disabled (set BOT_ENABLED=true to enable)");
    }
