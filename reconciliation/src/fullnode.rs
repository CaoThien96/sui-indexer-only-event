...
impl FullnodeClient {
    ...
    pub async fn count_events_in_window(
        ...
        let keys = self
            .list_event_keys_in_window(move_event_type, start_ms, end_ms)
            .await?;
        Ok(keys.len() as u64)
    }
...
mod tests {
    ...
    #[test]
    fn parses_timestamp_string() {
        let event = json!({ "timestampMs": "1780672063438" });
        assert_eq!(parse_timestamp_ms(&event), Some(1780672063438));
    }
}
