# 04 — Hợp đồng dữ liệu (Data Contracts)

Kafka topics, schema chuẩn hóa, và bảng DB cốt lõi.  
**Phiên bản:** 1.0 — chốt cùng Phase 1

> **English:** [../04-data-contracts.md](../04-data-contracts.md)

---

## 1. Kafka topics

| Topic | Phase | Producer | Consumer | Retention |
|-------|-------|----------|----------|-----------|
...
## 2. Payload `dex.swap.raw.v1`

Indexer publish sau BCS decode. `parsed_json` dùng tên field gốc của protocol.
...
## 3. Payload `dex.swap.normalized.v1`

Do `swap-normalizer` tạo. Mọi engine downstream chỉ consume topic này.
...
**Khóa idempotency:** `(tx_digest, event_seq, protocol)`

**Quy tắc giá (đã chốt):**
- `quote_coin_type` = SUI hoặc USDC nếu pool có một trong hai; không thì `coin_type_a` làm quote
- `price_quote_per_base` tính từ `sqrt_price_after`, decimals, và `a_to_b`

---

## 4. Payload `dex.pool.raw.v1`
...
## 5. Payload `token.metadata.raw.v1`
...
## 6. Payload `coin.balance_change.v1` (Phase 4)
...
## 7. Bảng Postgres (production)

**Không có bảng `package_events` trong production.** Raw fact nằm trên Kafka (BYOS chính). Postgres chỉ watermark + catalog.

### `watermarks` (bắt buộc bởi sui-indexer-alt)

Quản lý bởi `crates/indexer-store` — một row mỗi pipeline. Xem [BYOS](https://docs.sui.io/develop/accessing-data/custom-indexer/bring-your-own-store).
...
## 9. Bảng ClickHouse (cold)

Mirror `swaps_fact`, `ohlc_1m`, và (Phase 5) `transfer_edges`:
...
|-------------|-----|----------|
| `token:{coin_type}:price` | 60s | giá mới nhất + pool_id |
| `token:{coin_type}:vol:24h` | 120s | volume + số giao dịch |
| `pool:{pool_id}:tvl` | 300s | ước tính TVL |

---

## 11. Hình dạng response API (Phase 2)
...
`holder_count` được điền từ Phase 4.
